...
export function CombatResultModal() {
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
          <div className="bg-slate-800 border border-slate-600 rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
            ...
            <div className="text-center mb-4">
              <div className="text-lg mb-2">{winnerText}</div>
              {!lastCombatResult.dangerousActivated && (
                <div className="flex justify-center gap-8 mb-2">
                  <div className="text-center">
                    <div className="text-sm text-slate-400">Attacker</div>
                    <div className="text-xl">{lastCombatResult.attackerTotal}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-sm text-slate-400">Defender</div>
                    <div className="text-xl">{lastCombatResult.defenderTotal}</div>
                  </div>
                </div>
              )}
  ...
  if (combatPhase === 're-roll') {
    const { attackerRoll, defenderRoll, attackerTotal, defenderTotal, rerollsUsed,
            ...
            defenderCanCruel, defenderCanRelentless } = pendingCombat;
  ...
  if (combatPhase === 'resolution') {
    const { attackerTotal, defenderTotal } = pendingCombat;
